...
awk '{if (NF==22 && $1==1)   print $12,$11,$4/12}' "$clust" | gmt psxy -R -JM -Sc -W1,red -K -O >> "$ps"
awk '{if (NF==22 && $1==2)   print $12,$11,$4/12}' "$clust" | gmt psxy -R -JM -Sc -W1,blue -K -O >> "$ps"
awk '{if (NF==22 && $1==1.5) print $12,$11,$4/12}' "$clust" | gmt psxy -R -JM -Sc -W1,green -K -O >> "$ps"
awk '{if (NF==22 && $1>1.5)  print $12,$11,$4/12}' "$clust" | gmt psxy -R -JM -Sc -W1,195/214/25 -K -O >> "$ps"

# plot focal mechanism
gmt psmeca 201807030920.foc -R -JM -Sa1.0/14p/6 -Gred -O -K >> "$ps"

# plot centroid of earthquakes cluster
awk '{if (NF==6 && $1<4) print $4,$3,0.3}' "$clust" | gmt psxy -R -JM -Sa -W1,purple -Gyellow -K -O >> "$ps"
...
awk '{print $5,$3,$4/12}' catalog_profile.gmt | gmt psxy -R -JX -Sc -W1,red -K -O >> "$ps"

# cluster No.2
#awk '{printf("%f %f %f %f\n",$9,$8,$10,$11)}' "$clust" | \
awk '{if(NF==22 && $1==2) printf("%f %f %f %f\n",$12,$11,$13,$4)}' "$clust" | \
gmt project -C"$A1_lon"/"$A1_lat" -E"$A2_lon"/"$A2_lat" -W-"$width"/"$width" -Q > catalog_profile.gmt
awk '{print $5,$3,$4/12}' catalog_profile.gmt | gmt psxy -R -JX -Sc -W1,blue -K -O >> "$ps"

# cluster No.3
#awk '{printf("%f %f %f %f\n",$9,$8,$10,$11)}' "$clust" | \
awk '{if(NF==22 && $1==3) printf("%f %f %f %f\n",$12,$11,$13,$4)}' "$clust" | \
gmt project -C"$A1_lon"/"$A1_lat" -E"$A2_lon"/"$A2_lat" -W-"$width"/"$width" -Q > catalog_profile.gmt
awk '{print $5,$3,$4/12}' catalog_profile.gmt | gmt psxy -R -JX -Sc -W1,green -K -O >> "$ps"

# other clusters
#awk '{printf("%f %f %f %f\n",$9,$8,$10,$11)}' "$clust" | \
awk '{if(NF==22 && $1>3) printf("%f %f %f %f\n",$12,$11,$13,$4)}' "$clust" | \
gmt project -C"$A1_lon"/"$A1_lat" -E"$A2_lon"/"$A2_lat" -W-"$width"/"$width" -Q > catalog_profile.gmt
awk '{print $5,$3,$4/12}' catalog_profile.gmt | gmt psxy -R -JX -Sc -W1,195/214/25 -K -O >> "$ps"

# plot centroid of earthquakes cluster
awk '{if (NF==6 && $1<4) printf("%f %f %f %f\n",$4,$3,$5,6)}' "$clust" | \
gmt project -C"$A1_lon"/"$A1_lat" -E"$A2_lon"/"$A2_lat" -W-"$width"/"$width" -Q > catalog_profile.gmt
awk '{print $5,$3,$4/12}' catalog_profile.gmt | gmt psxy -R -JX -Sa -W1,purple -Gyellow -K -O >> "$ps"
...
echo "2 2 0.5" | gmt psxy -R -J -Sc -W1,red -K -O >> "$ps"
echo "2 3 0.5" | gmt psxy -R -J -Sc -W1,blue -K -O >> "$ps"
echo "2 4 0.5" | gmt psxy -R -J -Sc -W1,green -K -O >> "$ps"
echo "2 5 0.5" | gmt psxy -R -J -Sc -W1,195/214/25 -K -O >> "$ps"
echo "2 6 0.5" | gmt psxy -R -J -Sa -W1,purple -Gyellow -K -O >> "$ps"
...
echo "3 6 cluster 1~3 centroid" | gmt pstext -R -J -O `#-K` -F+f14p+jML >> "$ps"
